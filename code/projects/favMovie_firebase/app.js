...
function MainController($scope, $firebase) {
    $scope.favMovies = $firebase(new Firebase('https://myreactproject1.firebaseio.com/movies'));
    $scope.movies = [];
...
function buildEndPoint(key, $firebase) {
    return $firebase(new Firebase('https://myreactproject1.firebaseio.com/movies/' + key));
}
